...
(fact "Generating lazy sequences"
    ...
    (take 10 (iterate identity :again)) => (repeat 10 :again)
             )

(fact "Sequence comprehensions"
    ...
    (for [index (range 5)] index) => '(0 1 2 3 4)

    ;remember the binding is immutable so inc only modifies what is returned
    (for [index (range 5)] (inc index)) => '(1 2 3 4 5)

    ;when lets you filter what's yielded
    (for [index (range 10) :when (odd? index)] index) => '(1 3 5 7 9)

    (for [index (range 10) :when (even? index)] (dec index)) => '(-1 1 3 5 7)

    ;lets really push the boat out, multiple pairs with a while modifier and a function applied
    (take 10 (for [x (range 1000000) y (range 1000000) :while (< y x)] (* x y))) => '(0 0 2 0 3 6 0 4 8 12)
             )
